...
use super::parser::{parse_captures, parse_captures_iter, Captures};
use super::errors::{PemError, Result};
use crate::ensure;
...
fn decode_data(raw_data: &str) -> Result<Vec<u8>> {
    ...
    let contents = near_sdk::base64::engine::general_purpose::STANDARD
        .decode(data)
...
pub fn encode_config(pem: &Pem, config: EncodeConfig) -> String {
    ...
    let contents = if pem.contents.is_empty() {
        ...
        near_sdk::base64::engine::general_purpose::STANDARD.encode(&pem.contents)
    };
