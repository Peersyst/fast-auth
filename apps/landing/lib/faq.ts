...
export const FAQ_ITEMS: ReadonlyArray<FaqItem> = [
  {
    q: "Which sign-in methods does FastAuth support?",
  ...
  {
    q: "How does FastAuth compare to Privy, Reown, or Web3Auth?",
    a: "FastAuth is NEAR-native and non-custodial via MPC. Where embedded-wallet products like Privy, Reown (formerly WalletConnect), or Web3Auth rely on proprietary infrastructure (key shards held by the provider, MPC services bound to their stack, or session-key wallets), FastAuth derives a real NEAR named account directly inside NEAR's MPC network from the user's Auth0-verified identity — no single party ever reconstructs the full key. Transactions are dispatched as NEP-366 meta-transactions through an open relayer your dApp configures, and the JWT guard model lets you bring your own issuer if Auth0 doesn't fit. The result: same Google/Apple/email login UX, but on NEAR rails you can audit end-to-end.",
  },
  {
