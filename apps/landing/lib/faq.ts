...
export const FAQ_ITEMS: ReadonlyArray<FaqItem> = [
  {
    q: "Which sign-in methods does NEAR Auth support?",
  ...
  {
    q: "How does NEAR Auth compare to Privy, Reown, or Web3Auth?",
    a: "NEAR Auth is NEAR-native and non-custodial via MPC. Where embedded-wallet products like Privy, Reown (formerly WalletConnect), or Web3Auth rely on proprietary infrastructure (key shards held by the provider, MPC services bound to their stack, or session-key wallets), NEAR Auth derives a real NEAR named account directly inside NEAR's MPC network from the user's Auth0-verified identity — no single party ever reconstructs the full key. Transactions are dispatched as NEP-366 meta-transactions through an open relayer your dApp configures, and the JWT guard model lets you bring your own issuer if Auth0 doesn't fit. The result: same Google/Apple/email login UX, but on NEAR rails you can audit end-to-end.",
  },
  {
    q: "What chain does NEAR Auth run on?",
    a: "NEAR Protocol mainnet. NEAR Auth is built and operated by Peersyst as a public-good account-abstraction layer for the NEAR ecosystem. Accounts are real NEAR named accounts you can inspect on nearblocks.io.",
  },
  {
    q: "How do users sign in?",
    a: "Through Auth0 — Google, Apple, email/password, or passkey. The user's MPC-controlled NEAR account is derived deterministically from their Auth0 identity, so the same login always resolves to the same account no matter which NEAR Auth dApp they sign into.",
  },
  {
    q: "What does Auth0 have to do with it?",
    a: "Auth0 is the identity layer. Each NEAR Auth-integrated dApp gets its own approved Auth0 credentials (domain, clientId, audience), but the user's `sub` claim deterministically derives the same MPC-controlled NEAR account across every dApp that uses the Auth0 guard — so the same login reaches the same wallet.",
  },
  {
    q: "Where is the user's key stored?",
    a: "User signs in. Auth0 issues a JWT. The MPC network derives the user's key from their JWT. The NEAR Auth contract routes the JWT to the matching guard (Auth0, Firebase, custom issuer) for cryptographic verification. On success it builds a deterministic path — {guard_id}#{sub} — and asks NEAR's MPC network to sign for it. The nodes derive the same key for the same identity every time, and produce the signature collaboratively. No single party ever holds the full key — and the user holds no key material at all.",
  },
  {
    q: "Is this custodial?",
  ...
  {
    q: "Can I self-host?",
    a: "Partially. The NEAR Auth contract, the JWT Guard Router, and NEAR's MPC network are shared infrastructure deployed on mainnet (fast-auth.near, jwt.fast-auth.near, v1.signer) — you integrate with them, you don't redeploy them. You can self-host the auth side: deploy your own guard contract that implements the JwtGuard trait, or run an off-chain issuer whose JWTs are verified by CustomIssuerGuard. Both extension points are documented.",
  },
  {
    q: "Has NEAR Auth been audited?",
    a: "Yes. The NEAR Auth smart contracts and signing infrastructure have been audited by Halborn, an independent security firm specializing in Web3. The full security report is publicly available.",
  },
];
