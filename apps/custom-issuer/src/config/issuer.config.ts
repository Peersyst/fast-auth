...
export default (): IssuerConfig => {
  ...
  try {
    ...
    throw new Error(
      `Invalid VALIDATION_PUBLIC_KEY_URL: "${validationPublicKeyUrl}" is not a valid URL.`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Invalid KEY_BASE64: ${error instanceof Error ? error.message : 'The value is not valid base64-encoded data.'}`,
    );
  }
