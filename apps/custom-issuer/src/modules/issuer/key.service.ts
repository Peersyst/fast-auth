...
export class KeyService implements OnModuleInit {
    ...
    private async loadPublicKeysFromUrl(url: string): Promise<string[]> {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch public keys from URL: ${response.status} ${response.statusText}`);
            }
            ...
            try {
                ...
                throw new Error(`Unable to parse public keys from URL. Expected PEM format or Firebase certificate format.`);
            }
        } catch (error) {
        ...
        try {
            ...
            for (const [keyId, value] of certificateEntries) {
                if (typeof value === "string" && value.includes("-----BEGIN")) {
                    ...
                    this.logger.log(`Loaded public key from Firebase URL (key-id: ${keyId})`);
                }
            }
            ...
            throw new Error(`Failed to parse Firebase public keys response: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    ...
    getSigningKey(): string {
        if (!this.signingKey) {
            throw new Error("Signing key not loaded. Check application startup logs.");
        }
