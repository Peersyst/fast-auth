...
class FastAuthRelayer {
    ...
    async sign(jwt: string) {
        ...
        try {
            return await this.account.functionCall({
                contractId: getFastAuthContractId(this.networkId),
                methodName: "sign",
    ...
    async derivePublicKey(sub: string) {
        const mpcContractId = this.networkId === "mainnet" ? "v1.signer" : "v1.signer-prod.testnet";
        const publicKey = await this.account.viewFunction({
            contractId: mpcContractId,
            methodName: "derived_public_key",
            args: { path: sub, predecessor: getFastAuthContractId(this.networkId) },
        });
        return publicKey;
    }
    ...
    async relaySignAction(action: Action) {
        ...
        const tx = createTransaction(
            ...
            getFastAuthContractId(this.networkId),
            nonce,
            [action],
            base_decode(accessKey.block_hash),
        );
