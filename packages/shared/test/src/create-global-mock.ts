...
export function createGlobalMock<C extends object>(
  ...
  new (
    customData?: Partial<MockDataMethods<C>>,
  ): ExtendedMock<C, jest.SpyInstance>;
} {
    ...
    constructor(customData: Partial<MockDataMethods<C>> = {}) {
      ...
      for (const [key, item] of Object.entries(data)) {
        if (item instanceof MethodMock) {
          const usedMethod =
            customData?.[key as keyof MockDataMethods<C>] || item;
          (this as any)[key] = jest
            .spyOn(obj, key as any)
            [usedMethod.type](usedMethod.value);
        } else {
